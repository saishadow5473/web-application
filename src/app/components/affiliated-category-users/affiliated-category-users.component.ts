...
@Component({
  selector: 'app-affiliated-category-users',
  templateUrl: './affiliated-category-users.component.html',
  styleUrls: ['./affiliated-category-users.component.css']
})

export class AffiliatedCategoryUsersComponent implements OnInit {
  ...
  userLocalData = JSON.parse(this._constant.aesDecryption('userData'));
  ...
  headerName:string = this._constant.teleSpecalityType;
  ...
  classData: Array<any> = [];
  cardExpendDivCourse = false;
  selectedCourse:any|string = '';
  headerNameCourse = 'Select Classes';
  ...
  selectedStarCourse:number = 0;
  sendReviewCourse:boolean = false;
  rateArrayCourse:number[] = [0,0,0,0,0];
  isValidationDone:boolean = false;
  noCourseIsAvailable:boolean = false;
  brand_image_url_exist_course:boolean = false;
  brand_image_url_course:string = '';
  subscriptionStartDate:string = "";
  @ViewChild('reviewFieldCourse') reviewFieldCourse: any;
  ...
  ngOnInit(): void {
    ...
    //For showing consultant List and course List if any one category is selected 
    if (this._constant.showAffCourseWithoutCategory == false) {
      //Redirecting to affiliation page selection if page refresh - identifies teleSpecalityType == null and length of consultant and course is '0'
      if (this._constant.filteredCoursesList.length == 0 && this._constant.filteredConsultantList.length == 0 && this._constant.teleSpecalityType == null) {
        this.router.navigate(['/affiliated-users']);
      }

      //For showing courseList
      if (this._constant.filteredCoursesList.length > 0) {
        this._constant.teleConsultPageFlow = ['Fitness Class'];
        if (this._constant.teleConsultPageFlow.length == 0) {
          this.zone.run(() => {
        ...
        // const consultantObject = this._constant.aesDecryption('consultantDataObj');
        ...
        //   this.router.navigate(['/fitnessPage']);
        // }
        // var consultantObj = {"ct":'Fitness Class' , "st":""};
        // localStorage.setItem("consultantDataObj" , this._constant.aesEncryption(JSON.stringify(consultantObj)));
        if (this._constant.teleconsultationFlowSelected == 'affiliate' && this._constant.consultationPlatformData != undefined) {
          const consultantObject = this._constant.aesDecryption('consultantDataObj')
          // const consultantObject = this._constant.aesDecryption(localStorage.getItem('consultantDataObj'));
          console.log(consultantObject)
          const parsedConsultantObject = JSON.parse(consultantObject);
          const consultanttype = parsedConsultantObject.ct;
          const specialityName = parsedConsultantObject.st;
          this.populateClasses(this._constant.consultationPlatformData, consultanttype, specialityName);
        } else if (this._constant.consultationPlatformData == undefined) {
          this.userData = JSON.parse(this._constant.aesDecryption("userData"));
          this.teleConsultService.getTeleConsultUserPlatformData(this.userData.id).subscribe(data => {
            // console.log(data);
            this._constant.consultationPlatformData = data;
            this.initializeCrossbarConnectionCourse();
            // this.populateClasses(this._constant.consultationPlatformData, consultanttype, specialityName);
          });
          this.zone.run(() => {
            //this.router.navigate(['/teleconsultation']);
      ...
      //For showing consultant List
      if (this._constant.filteredConsultantList.length > 0) {
        ...
        if (consultantObject != null) {
          var parsedConsultantObject = (consultantObject);
        ...
        this._constant.teleConsultationNewFlow = true;
    ...
    //For wellness care marketplace card dispalys course list - classes not comes under 4 pillar concept
    if (this._constant.showAffCourseWithoutCategory == true) {
      ...
      // if (consultanttype == undefined || specialityName == undefined) {
      //   //this.router.navigate(['/teleconsultation']);
      //   console.log("redirected to fitness")
      //   this.router.navigate(['/fitnessPage']);
      // }
      // var consultantObj = {"ct":'Fitness Class' , "st":""};
      // console.log("cons : ",consultantObj)
      // localStorage.setItem("consultantDataObj" , this._constant.aesEncryption(JSON.stringify(consultantObj)));
      if (this._constant.teleconsultationFlowSelected == 'affiliate' && this._constant.consultationPlatformData != undefined) {
        let consultantObject = this._constant.aesDecryption('consultantDataObj');
        // console.log(consultantObject,typeof consultantObject)
        // // const consultantObject = this._constant.aesDecryption(localStorage.getItem('consultantDataObj'));
        if(consultantObject == undefined || consultantObject == null || typeof consultantObject == 'object'){
          var consultantObj = {"ct":'Fitness Class' , "st":""};
          localStorage.setItem("consultantDataObj" , this._constant.aesEncryption(JSON.stringify(consultantObj)));
          consultantObject = JSON.parse( this._constant.aesDecryption('consultantDataObj'));
          // console.log(consultantObject)
        }
        // console.log(consultantObject)
        var parsedConsultantObject:any = consultantObject;
        var consultanttype = parsedConsultantObject.ct;
        var specialityName = parsedConsultantObject.st;
        this.populateClasses(this._constant.consultationPlatformData, consultanttype, specialityName);
      } else if (this._constant.consultationPlatformData == undefined) {
        this.userData = JSON.parse(this._constant.aesDecryption("userData"));
        this.teleConsultService.getTeleConsultUserPlatformData(this.userData.id).subscribe(data => {
  ...
  populatedoctorStatusMappingList():void{
    ...
    console.log(this._constant.fireStore);
  ...
  updateDoctorStatusListFromApi(api_res):void{
    ...
    for(let i=0; i<n; i++){
      try{
        ...
        // if((cur_date - _api_date) >= (15*60*1000)) continue; // hided to show offline,busy and online doctor
    ...
    for(let i=0; i<this.doctorList.length; i++){
      ...
      // console.log(this.doctorListNewArr);
    }
  ...
  goHome(){
    // console.log(this.showConsultant)
    // console.log(this.showCourse)
    // console.log(this._constant.isLoading)

    if((this.showConsultant == true || this.showCourse == true) && this._constant.isLoading == false){ //course or consultant list page move to 4 pillar shown page
      this.router.navigate(['/affiliated-users']);
    }else{
      // console.log("stopped");
    }  
    ...
    /* old code hided for back button functionality
  ...
  getDoctorList(consultType){
    ...
    let user = JSON.parse(this._constant.aesDecryption('userData'))
        ...
        if (consultations.consult_type[i].consultation_type_name == this.consultanttype) {
          if (consultations.consult_type[i].specality !== undefined && consultations.consult_type[i].specality !== null) {
            for (let j = 0; j < consultations.consult_type[i].specality.length; j++) {
              if (consultations.consult_type[i].specality[j].specality_name != undefined) {
                if (consultations.consult_type[i].specality[j].consultant_list !== undefined && consultations.consult_type[i].specality[j].consultant_list !== null) {
                  ...
                  if(this.consultantList !== undefined && this.consultantList !== null && this.consultantList !== "" && (Array.isArray(this.consultantList) && this.consultantList.length > 0) ){
                    ...
                    if(this._constant.teleconsultationFlowSelected == "affiliate"){
                      this.consultantList = this._constant.filteredConsultantList;
                      // this.hideModal();
                    }
                    console.log(this.consultantList);
                    if(this._constant.startCallFlow == true){
  ...
  showModal(message:string = '', spinner:boolean = true){
    ...
    // this.showLoadingModal = true; // need to show when necessary 
  }
  hideModal(){
  ...
  //For showing class List functionality starts here 
  initializeCrossbarConnectionCourse(){
  ...
  populateClasses(platformData, consultantType, specialityName):void{
    ...
    if(this._constant.filteredCoursesList == undefined){
    ...
    }else if(this._constant.filteredCoursesList.length > 0 && this._constant.showAffCourseWithoutCategory == false){
      this.classData = this._constant.filteredCoursesList;
    }else if(this._constant.overAllCourseList.length > 0 && this._constant.showAffCourseWithoutCategory == true){
      this.classData = this._constant.overAllCourseList;
    }else{
      this.classData = [];
    }
    ...
    if (this.classData.length > 0) {
      ...
      if (courses.length > 0) {
        if (this._constant.teleconsultationFlowSelected == "genric") {
          ...
          if(this._constant.teleconsultationAffiliationSelectedCompanyImg != '') {
            this.brand_image_url_exist_course = true;
            this.brand_image_url_course = this._constant.teleconsultationAffiliationSelectedCompanyImg;
          }
    ...
    if (this.classData.length > 0) {
      ...
      this.authService.getClassImages(classIdObj).subscribe(data => {
        ...
        this.userData = JSON.parse(this._constant.aesDecryption("userData"));
        this.teleConsultService.getTeleConsultUserData(this.userData.id).subscribe((data: any) =>{
          this._constant.consultationUserData = data;
          if('my_subscriptions' in data){
            ...
            // console.log(this._constant.consultationUserData);
  ...
  expandCard(course) {
    this.cardExpendDivCourse = true;
    this.isBookingCardOpen = true;
    this.isCourseDetailCardOpen = false;
    this.autoApproveCourseStatus = true;
    this.selectedCourse = course;
    this.headerNameCourse = 'Subscribe';
    this.prepareSelectedCourse();
  }
  shrinkCard() {
    // if(this.selectedCourse != '' && this.cardExpendDivCourse == true){
    //   this.headerNameCourse = 'Select Classes';
    //   this.cardExpendDivCourse = false;
    //   this.selectedCourse = '';
  ...
  expandReviewCard(course) {
    ...
    this.selectedCourse = course;
    this.headerNameCourse = 'Course Details';
  ...
  feedBackCourse(course){
    ...
    let userId =JSON.parse(this._constant.aesDecryption('userData'))['id'].toString();
    let ratings = this.selectedStarCourse.toString();
    let review = this.reviewFieldCourse.nativeElement.value.toString();
    ...
    this.authService.postCourseReview(obj).subscribe(data=>{
      ...
      if (typeof (data) === "object"){
        ...
        // let updatedReview:reviewObj = {rating_text:review, user_rating: this.selectedStarCourse, time_stamp: updatedReviewTimeStamp, user_name: userName};

        // course.ratings = ( Number(course.ratings) + this.selectedStarCourse) / 2;
        // course.text_reviews_data.push(updatedReview);

        this.reviewFieldCourse.nativeElement.value = "";
        this.rateArrayCourse = [0,0,0,0,0];
        this.selectedStarCourse = 0;
        this.sendReviewCourse = false;
  ...
  newDateOrderReviewsCourse(reviews: Array<any>): any{
    if (reviews.length > 1) {
      let sortedReviews = reviews.sort((a,b)=>{
        let timeSplitA = a.time_stamp.match(/\d+/);
        let timeSplitB = b.time_stamp.match(/\d+/);
  ...
  countStarsCourse(value) {
    if(this.selectedStarCourse == value){
      this.selectedStarCourse -= 1;
    }else{
      this.selectedStarCourse = value;
    }
    this.rateArrayCourse = [0,0,0,0,0];
    for (let i = 0; i < this.selectedStarCourse; i++) {
      this.rateArrayCourse[i] = 1;
    }
  }
  ...
  confirmSubscription():void{
    ...
    if(this.selectedCourse && 'course_fees' in this.selectedCourse){
      ...
      // console.log(this.selectedCourse);
      this._constant.selectedCourseConsultantId = this.selectedCourse.consultant_id;
      this._constant.selectedCourseId = this.selectedCourse.course_id;
      this._constant.consultationFee = this.course_fees*100;
      this.actualClassFee = this.course_fees;
      if(this.course_fees > 0){
        this._constant.showPaymentSelect = true;
        this._dialog.open(ModalComponent).afterClosed().subscribe(res => {
          console.log(res);
          if(res != undefined){
  ...
  checkUserDetailAndSub():void{
    if(this.selectedCourse && 'course_fees' in this.selectedCourse){
      ...
      var userData = JSON.parse(this._constant.aesDecryption('userData'));
      if(this.selectedTimeSlot === undefined ||  this.selectedTimeSlot === null ||  this.selectedTimeSlot.trim().length === 0){
        // alert("select time ")
        this.isLoading = false;
        this.snackBar.open("Please choose a time slot", '',{
  ...
  initiatePayment(){
    ...
    let requestID = this.teleConsultService.paymentRequestIdGenerate("web", course_id);
    let fee: number = 0;
    if (this._constant.teleconsultationFlowSelected == "affiliate") {
      let offerPrice:any = "";
      if(this.course_fees != undefined) offerPrice = (this.course_fees).toString();
      else offerPrice = this.selectedCourse['affilation_excusive_data']['affilation_array'][0]['affilation_price'];
      try{
        if(offerPrice == undefined || offerPrice == null || String(offerPrice).trim().length == 0) throw "Invalid OfferPrice";
      }catch(err){
        offerPrice = this.selectedCourse['affilation_excusive_data']['affilation_array'][0]['affilation_price'];
    ...
    this.teleConsultService.getPaymentOrderID(requestID, fee *100).subscribe(data=>{
      console.log(data);
  ...
  paymentProcessINitiate(order_id, key){
    ...
    //alert(appointmentBook);
     ...
     const options: any = {    
      ...
      "amount": fee, // Amount is in currency subunits. Default currency is INR. Hence, 50000 refers to 50000 paise    
      "currency": "INR",
    ...
    var rzp1 = new this.teleConsultService.nativeWindow.Razorpay(options);
    rzp1.open();   
  ...
  storePaymentTransactionDetails(razorPayResponse: any): void{
    ...
    console.log(jsonData);
    this.teleConsultService.paymentTransInit(jsonData).subscribe(data=>{
      console.log(data);
  ...
  callConfirmAppointment(){
    this._constant.createSubscriptionProcess = true;
    this.zone.run(() => {
      this._dialog.open(ModalComponent);
    });

    this.zone.run(() => {
      // var  jsonDataUpdate = {"ihl_id": this.userData.id,"total_amount": this.course_fees, "payment_status": "completed",  "transactionId": this.paymentTransactionId, "payment_for": "onlineClass", "MobileNumber":this.contactDetailsMobileNumber,  "payment_mode":"Online"};
      var  jsonDataUpdate = {"ihl_id": this.userData.id,"total_amount": this.course_fees, "payment_status": "completed",  "transactionId": this.paymentTransactionId, "payment_for": "onlineClass", "MobileNumber":this.contactDetailsMobileNumber};
      console.log(jsonDataUpdate);
      if(this.course_fees > 0) jsonDataUpdate["payment_mode"] = "Online";
      this.teleConsultService.paymentTransUpdate(jsonDataUpdate).subscribe(res=>{
        console.log(res);
      });
    });
    ...
    */
    ...
    if(this.selectedTimeSlot === undefined ||  this.selectedTimeSlot === null ||  this.selectedTimeSlot.trim().length === 0){
      ...
      this.authService.createUserSubscription(this.book).subscribe(data=>{
        resObj = data;
        this.zone.run(() => {
          this._constant.createSubscriptionProcess = false;
        ...
        if (initialRes.status == "Subscription Added") {
          ...
          this.zone.run(() => {
            // var  jsonDataUpdate = {"appointment_id": initialRes['subscription_id'], "ihl_id": this.userData.id,"total_amount": this.course_fees, "payment_status": "completed",  "transactionId": this.paymentTransactionId, "payment_for": "onlineClass", "MobileNumber":this.contactDetailsMobileNumber,  "payment_mode":"Online"};
            var  jsonDataUpdate = {"appointment_id": initialRes['subscription_id'], "ihl_id": this.userData.id,"total_amount": this.course_fees, "payment_status": "completed",  "transactionId": this.paymentTransactionId, "payment_for": "onlineClass", "MobileNumber":this.contactDetailsMobileNumber};
            if(this.course_fees > 0) jsonDataUpdate["payment_mode"] = "Online";
            console.log(jsonDataUpdate);
            this.teleConsultService.paymentTransUpdate(jsonDataUpdate).subscribe(res=>{
              console.log(res);
          ...
          if (this._constant.fireStore) {
            ...
            this.fireStoreService.create(initialRes['subscription_id'], this._constant.teleConsultationCollectionName, obj);
          } else {
            if(this.teleConsultCrossbarService.is_connected == true){
          ...
          if (this.course_fees != 0)
            this._constant.showSubscriptionDownloadInvoice = true;
            
          this._constant.showSubscriptionModal = true;
  ...
  downloadInvoice() {
    ...
    this.course_fees = this.book.course_fees;
    ...
    if(this.book.course_fees > 0) {
      this.deductedIgstAmt = (Number(this.book.course_fees) / 1.18).toFixed(2);
