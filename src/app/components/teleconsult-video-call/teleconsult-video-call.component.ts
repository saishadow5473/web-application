...
export class TeleconsultVideoCallComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    if (this._constant.fireStore)
      this.initializeFireStore(false);
    else
  ...
  initializeFireStore(getDocumentByID) {
    ...
    if (getDocumentByID) {
      const docRef = this.fireStoreService.getDocumentByID(this._appointment_id, this._constant.teleConsultationCollectionName);
  ...
  getSubscriptionDocument(subscriptionId) {
    const docRef = this.fireStoreService.getDocumentByID(subscriptionId, this._constant.subscriptionCollectionName);
    ...
    docRef.valueChanges().subscribe((doc) => {
      if (doc) {
        ...
        let res = doc;
        console.log(res);
        if (res['data']['status'] == 'ClassEndedByTrainer') {
  ...
  async initiateLiveAppointmentCall():Promise<void>{
    ...
    if (this._constant.fireStore) {
      ...
      console.log(obj);
      this.fireStoreService.create(this._appointment_id, this._constant.teleConsultationCollectionName, obj);
    ...
    this.teleconsultJitsiService._jitsi_onHangup = async ()=>{
      ...
      if(this._isCallEndedByDoctor == false){
        ...
        if (this._constant.fireStore) {
          ...
          this.fireStoreService.update(this._appointment_id, _data, this._constant.teleConsultationCollectionName);
  ...
  initiateFitnessClassAppointment():void{
    ...
    this._constant.videoWindow = true;
    this._constant.videoCallStart = true;
    this.getSubscriptionDocument(this._appointment_id);
